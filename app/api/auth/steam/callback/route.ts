...
export async function GET(request: Request) {
  ...
  await db
    ...
    .set({
      steamId64,
      steamPersonaName: check?.personaName ?? null,
      steamAvatarUrl: check?.avatarUrl ?? null,
      steamLinkedAt: now,
      cs2Minutes: check?.cs2Minutes ?? 0,
      status: check?.eligible ? "verified" : "pending",
    })
  ...
  await db
    ...
    .values({
      ...
      userId: user.id,
      steamId64,
      profilePublic: check?.profilePublic ?? false,
      gameDetailsPublic: check?.gameDetailsPublic ?? false,
      ownsCs2: check?.ownsCs2 ?? false,
      cs2Minutes: check?.cs2Minutes ?? 0,
      eligible: check?.eligible ?? false,
      rawSnapshotJson: check ? JSON.stringify(check) : null,
      checkedAt: now,
    });
  return Response.redirect(
    `${TENE_ORIGIN}/?steam=${check?.eligible ? "verified" : "linked"}`,
    302,
  );
}
