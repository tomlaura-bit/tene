...
export async function GET() {
  ...
  return Response.json({ ok: database, service: "tene-api", version: 2, checks: { database, uploads: Boolean(env.UPLOADS), steam: Boolean(runtime.STEAM_WEB_API_KEY), matchProvider: runtime.MATCH_PROVIDER === "dathost" && Boolean(runtime.DATHOST_GAME_SERVER_ID) ? "configured" : "pending" }, responseMs: Date.now() - started, timestamp: new Date().toISOString() }, { status: database ? 200 : 503, headers: { "cache-control": "no-store" } });
}
