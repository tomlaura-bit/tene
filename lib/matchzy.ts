...
export async function provisionMatch(input: { roomId: string; map: string; side: "ct" | "t"; players: Array<{ steamId64: string | null; team: string }> }) {
  ...
  const response = await fetch(`${config.apiUrl}/matches`, { method: "POST", headers: { authorization: `Bearer ${config.token}`, "content-type": "application/json" }, body: JSON.stringify({ externalId: input.roomId, map: input.map, side: input.side, players: input.players, webhookPath: "/api/integrations/matchzy/webhook" }) });
