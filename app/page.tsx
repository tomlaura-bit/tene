...
function RoomRow({ name, players: count, average, openRoom }: { name: string; players: string; average: string; openRoom: () => void }) {
  const roomProfiles = name.includes("Violeta") ? [{name:"hoxhi",level:10,elo:1842,hours:4260,conduct:"Excelente"},{name:"Tom",level:8,elo:1626,hours:1284,conduct:"Excelente"},{name:"Jericho",level:7,elo:1514,hours:2110,conduct:"Buena"},{name:"k1ng",level:6,elo:1438,hours:980,conduct:"Buena"}] : name.includes("Nocturna") ? [{name:"melo",level:10,elo:1791,hours:3890,conduct:"Excelente"},{name:"navi",level:8,elo:1654,hours:2740,conduct:"Buena"},{name:"shiro",level:7,elo:1532,hours:1980,conduct:"Excelente"},{name:"loko",level:6,elo:1441,hours:1105,conduct:"Buena"},{name:"ace",level:5,elo:1372,hours:840,conduct:"Buena"}] : [{name:"neo",level:4,elo:1218,hours:720,conduct:"Buena"},{name:"rayo",level:3,elo:1140,hours:610,conduct:"Excelente"}];
  const creator = roomProfiles[0];
  return <article className="room-row-rich"><div className="room-info"><div className="room-title-line"><span className="room-symbol">T</span><div><strong>{name}</strong><small><i /> Esperando jugadores</small></div></div><div className="room-created">Creada por <ProfileAvatar profile={creator} compact /> <b>{creator.name}</b></div><div className="room-economy"><span><small>ENTRADA</small>S/ 6</span><span><small>PREMIO</small>S/ 10</span><span><small>PROMEDIO</small>{average}</span></div></div><div className="room-people"><div className="room-capacity"><span>{count} jugadores</span><div className="mini-progress"><i style={{width:`${Number(count.split("/")[0].trim())*10}%`}} /></div></div><div className="avatar-strip">{roomProfiles.map((profile) => <ProfileAvatar key={profile.name} profile={profile} />)}{Array.from({length:Math.max(0,10-roomProfiles.length)},(_,i)=><span className="vacant-avatar" key={i} />)}</div></div><button className="room-enter" onClick={openRoom}>Entrar <span>→</span></button></article>;
}

function ProfileAvatar({ profile, compact = false }: { profile: {name:string;level:number;elo:number;hours:number;conduct:string}; compact?: boolean }) {
  return <span className={`profile-anchor ${compact ? "compact" : ""}`} tabIndex={0}><img src={`https://api.dicebear.com/9.x/thumbs/svg?seed=${profile.name}&backgroundColor=2e1065,312e81,164e63`} alt={`Avatar de ${profile.name}`} /><span className="profile-popover"><span className="profile-pop-head"><img src={`https://api.dicebear.com/9.x/thumbs/svg?seed=${profile.name}&backgroundColor=2e1065,312e81,164e63`} alt="" /><span><strong>{profile.name}</strong><small>Steam conectado · Verificado</small></span><b>LVL {profile.level}</b></span><span className="profile-stats"><span><small>ELO</small>{profile.elo}</span><span><small>HORAS CS2</small>{profile.hours.toLocaleString()}</span><span><small>CONDUCTA</small>{profile.conduct}</span></span><button>Ver perfil completo</button></span></span>;
}
